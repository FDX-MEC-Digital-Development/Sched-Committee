<template>
  <div class="space-y-8">
    ...
    <div v-else class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3 sm:p-4">
      ...
      <div class="sm:hidden">
        <button
          class="w-full flex items-center justify-between px-3 py-2 bg-gray-50 dark:bg-gray-700 rounded-md text-sm font-medium text-gray-700 dark:text-gray-300"
          @click="showMobileMenu = !showMobileMenu"
        >
          <span>Navigate to another section</span>
          <Icon :name="showMobileMenu ? 'heroicons:chevron-up' : 'heroicons:chevron-down'" class="h-4 w-4" />
        </button>
        ...
        <div v-show="showMobileMenu" class="mt-2 space-y-1">
          ...
          >
            <Icon :name="item.icon" class="h-4 w-4 mr-3" />
            {{ item.title }}
          </NuxtLink>
        </div>
      </div>
